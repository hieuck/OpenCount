...
final class CombinedTallyTests: XCTestCase {
    ...
    func testGlobalTallyWithOnlyManualMarkers() async throws {
        ...
        let (tallyA, tallyB) = await MainActor.run {
            let globalTally = Dictionary(
                grouping: session.markers,
                by: { $0.objectType.id }
            ).mapValues { $0.count }
            return (globalTally[typeA.id] ?? 0, globalTally[typeB.id] ?? 0)
        }
    ...
    func testGlobalTallyWithOnlyAIDerivedMarkers() async throws {
        ...
        let tallyA = await MainActor.run {
            let globalTally = Dictionary(
                grouping: session.markers,
                by: { $0.objectType.id }
            ).mapValues { $0.count }
            return globalTally[typeA.id] ?? 0
        }
    ...
    func testGlobalTallyWithMixedMarkers() async throws {
        ...
        let (tallyA, tallyB, totalTally) = await MainActor.run {
            let globalTally = Dictionary(
                grouping: session.markers,
                by: { $0.objectType.id }
            ).mapValues { $0.count }
            return (
                globalTally[typeA.id] ?? 0,
                globalTally[typeB.id] ?? 0,
                globalTally.values.reduce(0, +)
            )
        }
    ...
    func testGlobalTallySumEqualsTotalMarkerCount() async throws {
        ...
        let (tallySum, totalMarkers) = await MainActor.run {
            let globalTally = Dictionary(
                grouping: session.markers,
                by: { $0.objectType.id }
            ).mapValues { $0.count }
            return (globalTally.values.reduce(0, +), session.markers.count)
        }
